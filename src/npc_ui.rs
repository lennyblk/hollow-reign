...
fn show_page(out: &mut io::Stdout, data: &NpcData, text: &str, hint: &str, is_reward: bool) {
    ...
    // Séparateur vertical (2/3 pour texte, 1/3 pour ASCII)
    let split = (tw as usize * 2) / 3;
    ...
    // ── ASCII NPC (colonne droite) ────────────────────────────────────────────
    let ascii_x = (split + 3) as u16;
    for (i, line) in data.ascii.lines().enumerate() {
        let row = 2 + i as u16;
    ...
    // ── Dialogue (colonne gauche) ─────────────────────────────────────────────
    let text_w = split.saturating_sub(4);
    let text_color = if is_reward { Color::Green } else { Color::Grey };
    let mut row: u16 = 2;

    for line in wrap_str(text, text_w) {
