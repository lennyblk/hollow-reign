...
pub fn save(player: &Player, state: &NavigationState) {
    ...
    let data = SaveData {
        ...
        inventory: player.inventory.iter().map(|i| i.name().to_string()).collect(),
        eq_weapon:    eq.and_then(|e| e.weapon.as_ref()).map(|i| i.name().to_string()),
        eq_armor:     eq.and_then(|e| e.armor.as_ref()).map(|i| i.name().to_string()),
        eq_shield:    eq.and_then(|e| e.shield.as_ref()).map(|i| i.name().to_string()),
        eq_consumables: eq
...
pub fn load(world: &World) -> Option<(Player, NavigationState)> {
    ...
    if let Some(ref name) = data.eq_weapon  { if let Some(i) = item_by_name(name) { eq.equip(i); } }
    if let Some(ref name) = data.eq_armor   { if let Some(i) = item_by_name(name) { eq.equip(i); } }
    if let Some(ref name) = data.eq_shield  { if let Some(i) = item_by_name(name) { eq.equip(i); } }
    for name in &data.eq_consumables {
        if let Some(i) = item_by_name(name) { eq.equip(i); }
    }
